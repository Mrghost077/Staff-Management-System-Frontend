...
const Reports = () => {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      ...
      didDrawPage: () => {
        ...
        doc.setFont("helvetica", "normal");
        doc.text(`Date: ${today}`, 15, pageHeight - 20);
        const signatureText = "Principal / Section head";
        const dots = ".............................................................";
        const dotsWidth = doc.getTextWidth(dots);
        const textWidth = doc.getTextWidth(signatureText);
        doc.setFont("helvetica", "italic");
        doc.text(dots, pageWidth - dotsWidth - 15, pageHeight - 25);
        doc.setFont("helvetica", "bold");
        doc.text(signatureText, pageWidth - textWidth - 15, pageHeight - 20);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <div className={`flex-1 p-6 transition-opacity duration-300 ${loading ? "opacity-50" : "opacity-100"}`}>
        ...
        {activeTab === "attendance" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <StatCard title="Avg Attendance" value={attendanceData?.summary?.averageAttendance} icon={<Activity />} color="text-blue-600" />
            <StatCard title="Total Present" value={attendanceData?.summary?.totalPresent} icon={<UserCheck />} color="text-green-600" />
            <StatCard title="Total Late" value={attendanceData?.summary?.totalLate} icon={<Users />} color="text-yellow-600" />
            <StatCard title="Total Absent" value={attendanceData?.summary?.totalAbsent} icon={<Calendar />} color="text-red-600" />
          </div>
        )}
        ...
        {activeTab === "leave" ? (
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="lg:w-1/3">
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
                <h4 className="text-xs font-bold mb-6 uppercase text-gray-400 tracking-wider">Leave Distribution</h4>
                <div className="h-72">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        ...
                        dataKey="value"
                      >
                        {(leaveData?.distribution || []).map((e, i) => (
            ...
            <div className="lg:w-2/3">
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                  <h3 className="font-bold text-gray-800 text-lg">Leave Details</h3>
                  <div className="flex gap-2">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-5 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
              <h3 className="font-bold text-gray-800 capitalize text-lg">{activeTab} Details</h3>
              <div className="flex gap-2">
                <SearchInput value={searchTerm} onChange={setSearchTerm} />
                <DateInput value={filterDate} onChange={setFilterDate} />
              </div>
...
const StatCard = ({ title, value, icon, color }) => (
  <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
    <div className={`flex justify-between mb-2 ${color}`}>
      {icon} <span className="text-black font-bold text-xs uppercase">{title}</span>
    </div>
    ...
    <div className="overflow-x-auto">
      <table className="w-full text-left">
        <thead className="bg-gray-50 text-gray-400 text-[11px] uppercase font-bold tracking-widest border-b border-gray-100">
          <tr>
            {columns[type].map((c) => <th key={c} className="px-6 py-5">{c}</th>)}
          </tr>
        </thead>
              ...
              <tr key={idx} className="hover:bg-gray-50/50 text-sm">
                {type === "attendance" && (
                  <>
                    ...
                    <td className="px-6 py-4 font-semibold text-gray-800">{row.teacher}</td>
                ...
                {type === "relief" && (
                  <>
                    ...
                    <td className="px-6 py-4 text-gray-800 font-medium">{row.relief}</td>
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-[11px] font-bold ${styles[status] || "bg-gray-100 text-gray-600"}`}>
      {status}
    </span>
  );
};
